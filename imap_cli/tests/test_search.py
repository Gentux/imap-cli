...
class SearchTests(unittest.TestCase):
    ...
    def test_combine_search_criterion(self):
        ...
        search_criterion = [
            ...
            search.create_search_criterion_by_mail_address(mail_address, header_name='TO'),
        ]

        imap_search_request = search.combine_search_criterion(search_criterion)
        assert imap_search_request == '(BEFORE 03-Jan-1989 TO "user@example.tld")'

        imap_search_request = search.combine_search_criterion(search_criterion, operator='AND')
        assert imap_search_request == '(BEFORE 03-Jan-1989 TO "user@example.tld")'

        imap_search_request = search.combine_search_criterion(search_criterion, operator='OR')
        assert imap_search_request == 'OR BEFORE 03-Jan-1989 TO "user@example.tld"'

        imap_search_request = search.combine_search_criterion(search_criterion, operator='NOT')
        assert imap_search_request == 'NOT BEFORE 03-Jan-1989 TO "user@example.tld"'

        imap_search_request = search.combine_search_criterion(search_criterion, operator='WRONG')
        assert imap_search_request == '(BEFORE 03-Jan-1989 TO "user@example.tld")'
    ...
    def test_create_search_criteria_by_header(self):
        ...
        search_criterion = search.create_search_criterion_by_header("FROM", "user@example.tld")
    ...
    def test_fetch_mails_info(self):
        ...
        assert list(search.fetch_mails_info(self.imap_account, mail_set="42 24")) == reference_mails_info
    ...
    def test_search_cli_tools(self):
        ...
        sys.argv = ['imap-cli-search', '-t testTag,test2Tag -T testText', '-c', 'config-example.ini']
        assert search.main() == 0

        sys.argv = ['imap-cli-search', '-d', '2014-03-13', '-c', 'config-example.ini']
        assert search.main() == 0

        sys.argv = ['imap-cli-search', '-d', '201403-13', '-c', 'config-example.ini']
        assert search.main() == 0
